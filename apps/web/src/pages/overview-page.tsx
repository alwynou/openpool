...
export function OverviewPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader title="Overview" detail="A focused view of storage capacity, namespaces, and control-plane access." />
      ...
      <section className="grid divide-y divide-zinc-200 overflow-hidden rounded-lg border border-zinc-200 sm:grid-cols-2 sm:divide-x sm:divide-y-0 xl:grid-cols-4">
        <Metric icon={HardDrivesIcon} label="Storage accounts" value={String(accounts.length)} detail={`${healthy} healthy`} />
        <Metric icon={DatabaseIcon} label="Available capacity" value={formatBytes(Math.max(0, capacity - used))} detail={`${formatBytes(used)} used`} />
        <Metric icon={FolderSimpleIcon} label="Logical buckets" value={String(buckets.length)} detail="Stable namespaces" />
        <Metric icon={KeyIcon} label="Active API keys" value={String(activeKeys)} detail={`${keys.length} total`} />
      </section>

      <div className="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
        <section className="rounded-lg border border-zinc-200">
          <div className="flex items-center justify-between border-b border-zinc-200 px-5 py-4">
            <div><h2 className="text-sm font-semibold text-zinc-950">Storage pool</h2><p className="mt-1 text-xs text-zinc-500">Current provider health and utilization</p></div>
            <Link to="/accounts" className="inline-flex items-center gap-1 text-xs font-medium text-zinc-600 hover:text-zinc-950">Manage accounts<ArrowRightIcon className="size-3.5" aria-hidden /></Link>
          </div>
          {accounts.length ? (
              ...
              {accounts.slice(0, 5).map((account) => {
                ...
                return (
                  <div className="grid gap-3 px-5 py-4 sm:grid-cols-[1fr_auto_180px] sm:items-center" key={account.id}>
                    <div><p className="text-sm font-medium text-zinc-900">{account.name}</p><p className="mt-1 text-xs text-zinc-500">{account.provider.toUpperCase()} · priority {account.priority}</p></div>
          ...
          ) : <p className="px-5 py-12 text-center text-sm text-zinc-500">No storage accounts connected.</p>}
        </section>
        ...
        <section className="rounded-lg border border-zinc-200 p-5">
          ...
          <h2 className="mt-5 text-sm font-semibold text-zinc-950">Direct transfers enforced</h2>
          <p className="mt-2 text-sm leading-6 text-zinc-500">OpenPool handles metadata, placement, and signing. Object bytes move directly between clients and storage providers.</p>
          <div className="mt-6 flex items-center gap-2 text-xs font-medium text-zinc-700"><CheckCircleIcon className="size-4" weight="fill" aria-hidden />Worker data path stays clear</div>
        </section>
      </div>
    </div>
  );
