...
export function App() {
  ...
  const logoutMutation = useMutation({
    ...
    onError: (error) => toast.error('Sign out failed', { description: errorText(error) }),
  });
  ...
  return (
    <AppShell administrator={access.administrator} health={healthQuery.data ?? null} onLogout={() => logoutMutation.mutate()} logoutBusy={logoutMutation.isPending}>
      <Suspense fallback={<div className="grid min-h-80 place-items-center"><CircleNotchIcon className="size-5 animate-spin text-zinc-400" aria-label="Loading page" /></div>}>
        <Routes>
...
function AccessLoading({ health }: { readonly health: HealthResponse | null }) {
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="grid min-h-[calc(100vh-70px)] place-items-center px-5"><div className="text-center"><CircleNotchIcon className="mx-auto size-6 animate-spin text-zinc-500" aria-hidden /><p className="mt-3 text-sm text-zinc-500">Checking access…</p></div></div>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-zinc-50/50">
      ...
      <main className="mx-auto grid min-h-[calc(100vh-70px)] max-w-6xl items-center gap-12 px-5 py-12 lg:grid-cols-[1fr_460px] lg:px-8">
        <section className="max-w-xl">
          <span className="inline-flex items-center gap-2 rounded-md border border-zinc-200 bg-white px-2.5 py-1.5 text-xs font-medium text-zinc-600"><CloudCheckIcon className="size-4" aria-hidden />Cloudflare-native · Self-hosted</span>
          <h1 className="mt-7 text-5xl font-semibold tracking-[-0.055em] text-zinc-950 sm:text-6xl">One namespace for every object store.</h1>
          <p className="mt-6 max-w-lg text-base leading-7 text-zinc-500">Connect R2, B2, and S3-compatible storage to one control plane while object bytes move directly between clients and providers.</p>
        </section>
        {unavailableMessage ? (
          <section className="rounded-lg border border-zinc-200 bg-white p-7 shadow-sm"><LockKeyIcon className="size-6 text-zinc-500" aria-hidden /><h2 className="mt-5 text-xl font-semibold text-zinc-950">OpenPool is unavailable</h2><p className="mt-2 text-sm leading-6 text-zinc-500">{unavailableMessage}</p></section>
        ) : (
...
function AuthHeader({ health }: { readonly health: HealthResponse | null }) {
  return <header className="flex h-[70px] items-center justify-between border-b border-zinc-200 bg-white px-5 sm:px-8"><Brand /><span className="inline-flex items-center gap-2 text-xs font-medium text-zinc-500"><span className={`size-2 rounded-full ${health ? 'bg-zinc-950' : 'bg-zinc-300'}`} />{health ? `Control plane ${health.status}` : 'Control plane unavailable'}</span></header>;
}
...
function AuthCard({
  ...
  return (
    <section className="rounded-lg border border-zinc-200 bg-white p-7 shadow-sm">
      <p className="text-xs font-semibold tracking-[0.08em] text-zinc-500 uppercase">{view === 'setup' ? 'First-time setup' : 'Administrator access'}</p>
      <h2 className="mt-3 text-2xl font-semibold tracking-tight text-zinc-950">{view === 'setup' ? 'Create your administrator' : 'Sign in to OpenPool'}</h2>
      <p className="mt-2 text-sm leading-6 text-zinc-500">{view === 'setup' ? 'The bootstrap token is read once and never stored in the browser.' : 'Use the administrator credentials configured for this control plane.'}</p>
      <form className="mt-6 grid gap-4" onSubmit={submit}>
        <Field label="Username"><Input name="username" autoComplete="username" minLength={view === 'setup' ? 3 : undefined} required /></Field>
        <Field label="Password"><Input name="password" type="password" autoComplete={view === 'setup' ? 'new-password' : 'current-password'} minLength={view === 'setup' ? 12 : undefined} maxLength={256} required /></Field>
        {view === 'setup' ? <Field label="Bootstrap token"><Input name="bootstrapToken" type="password" autoComplete="off" required /></Field> : null}
        {message ? <p className="rounded-md border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-xs leading-5 text-zinc-700" role="status">{message}</p> : null}
        <Button type="submit" className="mt-1 w-full" busy={busy}>{view === 'setup' ? 'Initialize OpenPool' : 'Sign in'}<ArrowRightIcon className="size-4" aria-hidden /></Button>
      </form>
    </section>
  );
}
